...
%             0, 0, 1];
     ...
     disp(size(des_theta));
     disp(size(clusterTheta));
     dtheta = double(des_theta);
     ctheta = double(clusterTheta);
     thetacdot = dtheta*1 + (-1*ctheta);
...
%         yc_dot;                                 %26
%         ];
    ClusterStateSpaceVector = zeros(26, 1) %ClusterStateSpaceVector';
end